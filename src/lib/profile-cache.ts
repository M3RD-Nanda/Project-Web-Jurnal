 ...
 */
export function getCachedProfile(sessionId: string): UserProfile | null {
  try {
 ...
 * Cache profile data
 */
export function setCachedProfile(profile: UserProfile, sessionId: string): void {
  try {
    ...
    localStorage.setItem(CACHE_CONFIG.PROFILE_CACHE_KEY, JSON.stringify(cachedData));
...
export function setCachedSession(sessionData: any): void {
  try {
    ...
    sessionStorage.setItem(CACHE_CONFIG.SESSION_CACHE_KEY, JSON.stringify(cacheData));
 ...
 */
export function preloadProfileData(sessionId: string, fetchFunction: () => Promise<UserProfile | null>): void {
