...
export async function updateProfileAdminAction(
  ...
  if (
    authMetadata &&
    (authMetadata.first_name !== undefined ||
      authMetadata.last_name !== undefined)
  ) {
    ...
    if (authUpdateError) {
      console.error(
        "Server Action: Error updating auth user metadata:",
        authUpdateError
      );
  ...
  const { error } = await supabaseAdmin
    ...
    .upsert(
      ...
      { onConflict: "id" }
    ) // Use onConflict for upsert behavior
    .select()
    .single();

  if (error) {
    console.error(
      `Server Action: Error updating profile for user ${userId}:`,
      error
    );
  ...
  if (fetchError) {
    console.error(
      "Server Action: Error re-fetching updated profile:",
      fetchError
    );
    return { data: null, error: new Error(fetchError.message) };
  }

  const { data: authUserAfterUpdate, error: authUserError } =
    await supabaseAdmin.auth.admin.getUserById(userId);
  if (authUserError) {
    console.error(
      "Server Action: Error fetching auth user after update:",
      authUserError
    );
