...
export async function GET(request: Request) {
  ...
  if (authError || !user) {
    console.error(
      "API Admin Announcements GET: Auth error or no user:",
      authError?.message
    );
    return NextResponse.json(
  ...
  if (profileError || profile?.role !== "admin") {
    console.error(
      "API Admin Announcements GET: Profile error or not admin:",
      profileError?.message || "Role not admin"
    );
    return NextResponse.json(
...
export async function POST() {
  return NextResponse.json(
    { message: "POST operation is now handled by Server Actions." },
    { status: 200 }
  );
}
